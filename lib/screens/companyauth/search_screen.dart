...
class _SearchScreenState extends State<SearchScreen> with SingleTickerProviderStateMixin {
  ...
  if (response.statusCode == 400) {
    ...
    setState(() {
      if(filter == "Current Month"){
          monthLeaveRequests = requestedLeaves.map((json) => LeaveRequest.fromJson(json)).toList();;
        }else if(filter =="Tomorrow"){
          tomorrowLeaveRequests = requestedLeaves.map((json) => LeaveRequest.fromJson(json)).toList();;
        }else if(filter == "Current Week"){
          weekLeaveRequests = requestedLeaves.map((json) => LeaveRequest.fromJson(json)).toList();;
        }
      isLoading = false;    
    });
    if(filter == "Current Month"){
                    ...
                    child: Column(
                      children: [
                          ...
                          children: [
                            ElevatedButton(
                              onPressed: (){
                                ...
                                print(startDate);
                                print(endDate);
                                _getLeaveRequestByDate(token!, companyId.toString());
                              }, 
                       ...
                       isFilter? Column(
                          ...
                          children: [
                            Text("Search By", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold),),
                          ],
                        ),
            ...
            children: [
              ...
              SizedBox(
                ...
                child: Expanded(
                  child: TabBarView(
                    children: [
                      Center(child: tomorrowLeaveRequests.isNotEmpty? ListView.builder(
              itemCount: tomorrowLeaveRequests.length,
              itemBuilder: (context, index) {
               LeaveRequest leave = tomorrowLeaveRequests[index]; 
                return LeaveRequestCard(leave: leave);
              }, ): const Text("No Leave Requests for tomorrow") ),
                      Center(child: weekLeaveRequests.isNotEmpty? ListView.builder(
              itemCount: weekLeaveRequests.length,
              itemBuilder: (context, index) {
               LeaveRequest leave = weekLeaveRequests[index]; 
                return LeaveRequestCard(leave: leave);
              }, ): const Text("No Leave Requests for this week") ),
                      Center(child: monthLeaveRequests.isNotEmpty? ListView.builder(
              itemCount: monthLeaveRequests.length,
              itemBuilder: (context, index) {
               LeaveRequest leave = monthLeaveRequests[index]; 
                return LeaveRequestCard(leave: leave);
              }, ): const Text("No leave requests for this month") ),
                    ],
                  ),
                ),
              ),
                          ...
                          children: [
                            Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
                                Text("$startDate to $endDate", style: const TextStyle(fontSize: 16,fontWeight: FontWeight.bold),),
                            ...
                            SizedBox(
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  LeaveRequest leave = dateLeaveRequests[index];
                                  return LeaveRequestCard(leave: leave);
                                },
                                 ),
                            ): const Text("No leave requests for this date")
                          ],
                        )
                          ],
          )
